  ...
  async create(data: {
    ...
    for (const item of items) {
      ...
      try {
        ...
        else if(stockout.backorderId){
               const backorder = await this.prisma.backOrder.findUnique({
          where: { id: stockout.backorderId ?? '' },
        });

        if (!backorder) throw new Error('Related backorder not found');

        // Update StockIn quantity
        await this.prisma.backOrder.update({
          where: { id: backorder.id },
          data: {
            quantity: (backorder.quantity ?? 0) - quantity,
          },
        });
        }
