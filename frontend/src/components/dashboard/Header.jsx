    ...
    <header className="bg-white border-b border-primary-200 ">
      <div className="px-6 py-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-gray-900 -ml-2 "> Welcome to Umusingi Hardware</h1>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 ">
              {user?.profileImg ? (
                <img src={`${API_URL}${user.profileImg}`} className="w-8 h-8 rounded-full" />
              ) : (
                <User className="w-6 h-6 text-gray-600 " />
              )}
              <ChevronDown className={`w-4 h-4 text-gray-500 transition ${isDropdownOpen && 'rotate-180'}`} />
            </button>
