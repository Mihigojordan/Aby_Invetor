...
export const EmployeeAuthContextProvider = ({ children }) => {
    ...
    const subscribeToNotifications = async (label) => {
        if (!user?.id) {
            throw new Error('Employee must be logged in to subscribe');
        }
        ...
        try {
            ...
            console.log('Employee subscription:', subscription);
    ...
    const unsubscribeFromNotifications = async () => {
        if (!user?.id) {
            throw new Error('Employee must be logged in');
        }
    ...
    // 📋 Get all subscriptions for current employee
    ...
    useEffect(() => {
        const autoSubscribe = async () => {
            ...
            // Check if employee wants auto-subscribe
